...
import BaalTribute from '@/components/home/BaalTribute'

export default function Footer() {
  const pathname = usePathname()

  // 도구 페이지에서는 숨김 (자체 UI 있음)
  if (pathname.startsWith('/tools/')) return null

  return (
    <footer className="mt-8 pt-4 border-t border-baal-border max-w-[1200px] mx-auto px-5 pb-6">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 justify-center text-sm text-baal-text-light mt-2">
        ...
        <Link href="/privacy" className="hover:text-baal-gold transition-colors">개인정보처리방침</Link>
      </div>
      <BaalTribute />
    </footer>
  )
