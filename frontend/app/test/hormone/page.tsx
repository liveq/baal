...
const EMBED_URL = 'https://hormone.baal.co.kr/'
const EMBED_ORIGIN = 'https://hormone.baal.co.kr'
...
export default function HormonePage() {
  ...
  useEffect(() => {
    ...
    const sharedR = new URLSearchParams(window.location.search).get('r')
    if (sharedR) params.set('r', sharedR)
    setSrc(`${EMBED_URL}?${params.toString()}`)

    const onMessage = (e: MessageEvent) => {
      if (e.origin !== EMBED_ORIGIN) return
      const data = e.data as { type?: string; height?: number } | null
      if (data?.type === 'hormone-height' && typeof data.height === 'number') {
        setHeight(Math.max(600, Math.ceil(data.height) + 10))
      }
    }
