...
window.findZodiac = function() {
    ...
    if (month < 1 || month > 12 || day < 1 || day > 31) {
        showToastMessage('올바른 날짜를 입력해주세요 📅');
        return;
    }
...
window.loadYearlyFortune = async function(zodiacId) {
    ...
    // 최고의 달과 주의할 달
    document.getElementById('yearBestMonths').textContent = fortune.bestMonths.join(', ');
    document.getElementById('yearChallengingMonths').textContent = fortune.challengingMonths.join(', ');
}
...
window.checkCompatibility = async function() {
    if (!currentZodiacId) {
        showToastMessage('먼저 자신의 별자리를 선택해주세요 ⭐');
        return;
    }
    ...
    if (!partnerZodiacId) {
        showToastMessage('상대방의 별자리를 선택해주세요 💕');
        return;
    }
    ...
    // 조언
    document.getElementById('compatAdvice').textContent = compatibility.advice;
...
window.checkBirthdateCompatibility = async function() {
    ...
    if (!myMonth || !myDay) {
        showToastMessage('내 생년월일을 먼저 입력해주세요 😊');
        return;
    }
    
    if (!partnerMonth || !partnerDay) {
        showToastMessage('상대방 생년월일을 입력해주세요 💕');
        return;
    }
...
window.zodiacDescriptions = {
    1: {
        ...
        element: "불의 별자리",
        ruler: "화성",
        description: `황금양털을 찾아 떠난 아르고호의 모험처럼, 양자리는 끝없는 개척정신으로 새로운 세상을 열어갑니다.\n\n` +
  `전쟁의 신 마르스(화성)의 뜨거운 에너지를 받아 태어난 불의 전사들. 이들에게 '불가능'이라는 단어는 존재하지 않습니다.\n\n` +
  `🔥 **양자리 라이프 스타일**\n\n` +
  `• 엘리베이터 닫기 버튼을 광속으로 누르는 성격\n` +
  `• 신호등 바뀌면 가장 먼저 출발하는 스피드왕\n` +
  `• 카페에서 메뉴 고민 시간은 최대 3초\n` +
  `• "에이, 해보면 되지!"가 입에 붙은 개척자\n\n` +
  `친구가 "이거 어려울 것 같은데..."라고 말하는 순간.\n\n` +
  `벌써 "에이, 해보면 되지!"라며 앞장서고 있는 타고난 리더. 연애할 때는 "우리 사귀자!" 한 번에 끝내버리는 직진 로맨티스트입니다.`,
        ...
        🦁 <strong>사자자리</strong>: 불의 형제들! 양자리와 사자자리는 마치 액션 영화의 주인공 듀오 같습니다. 둘 다 "일단 저질러!"를 외치며 모험을 떠나고, 서로의 열정에 더욱 불타오릅니다. 데이트할 때도 "오늘 뭐 할까?"라는 질문에 동시에 "새로운 곳!"이라고 답하는 찰떡궁합입니다.
        
        🏹 <strong>사수자리</strong>: 모험 파트너! 양자리의 즉흥성과 사수자리의 자유로운 영혼이 만나 "세상은 넓고 할 일은 많다"를 실천합니다. 금요일 저녁 갑작스런 "내일 부산 가자!" 제안에도 "좋아!"하고 따라나서는 환상 콤비입니다.
        
        👥 <strong>쌍둥이자리</strong>: 에너지 폭발 듀오! 양자리의 추진력과 쌍둥이자리의 아이디어가 만나면 "이런 건 어때?"가 "바로 해보자!"로 이어지는 실행력 만점 커플입니다. 함께 있으면 지루할 틈이 없는 활기찬 관계입니다.
        
        🌟 <b>특별한 인연</b>
        
        ⚖️ <strong>천칭자리</strong>: 정반대의 매력! 양자리가 "이거야!"하면 천칭자리가 "잠깐, 다른 건 어때?"하며 서로 다른 매력으로 끌리는 복잡한 인연입니다. 양자리는 신중함을, 천칭자리는 결단력을 배우게 되는 성장 관계입니다.
        
        💔 <b>조심할 궁합</b>
        
        🦀 <strong>게자리</strong>: 물과 불의 만남은 증기만 만들어냅니다. 양자리가 "빨리 결정하자!"하면 게자리는 "좀 더 생각해볼까?"하며 속도 차이로 서로 답답해합니다. 하지만 양자리가 조금만 기다려주고 게자리가 조금만 빨라지면 의외로 좋은 조합이 될 수 있어요.
        
        🌾 <strong>처녀자리</strong>: 양자리의 "대충대충"과 처녀자리의 "완벽하게"가 충돌해 "왜 이렇게 급해?" vs "왜 이렇게 꼼꼼해?"를 주고받는 관계입니다. 서로의 방식을 이해하려는 노력이 필요해요.
        
        🤝 <b>우정으로는 최고</b>
        
        🐐 <strong>염소자리</strong>: 연애로는 속도 차이가 있지만, 친구로는 환상적입니다. 양자리의 아이디어를 염소자리가 현실적으로 다듬어주는 든든한 친구 관계예요.`
    },
    2: {
        name: "황소자리 (Taurus)",
        ...
        element: "흙의 별자리",
        ruler: "금성",
        description: `미의 여신 비너스(금성)의 축복을 받아 태어난 황소자리. 세상의 모든 아름답고 달콤한 것들을 사랑하는 감각의 예술가입니다.\n\n` +
  ...
  `자신만의 여유로운 템포로 살아가는 사람들. 연애할 때는 "맛있는 거 해줄게"라는 한마디로 상대방의 마음을 사로잡는 현실적인 로맨티스트입니다.`,
        ...
        🌾 <strong>처녀자리</strong>: 흙의 신뢰할 수 있는 파트너! 황소자리가 "이 집 괜찮지 않아?"하면 처녀자리가 "구조와 채광, 교통편까지 완벽해"라며 체계적으로 분석해주는 든든한 관계입니다. 함께 집꾸미기를 하면 인테리어 잡지에 나올 법한 완벽한 공간을 만들어내는 실속형 커플입니다.
        
        🐐 <strong>염소자리</strong>: 현실적인 파워 커플! 황소자리의 안정 추구와 염소자리의 계획성이 만나 "우리 10년 후엔 어떻게 살까?"를 진짜로 실현시키는 조합입니다. 데이트 코스도 "맛있고 분위기 좋고 가격도 합리적인" 완벽한 장소를 찾아내는 모범생 커플입니다.
        
        🦀 <strong>게자리</strong>: 안정과 따뜻함의 만남! 황소자리의 현실감각과 게자리의 감성이 만나 "집이 이렇게 편할 수가 있구나"를 느끼게 해주는 홈 스위트 홈 커플입니다. 함께 요리하고 영화보며 집에서 보내는 시간이 최고의 데이트인 관계입니다.
        
        🌟 <b>특별한 인연</b>
        
        🦂 <strong>전갈자리</strong>: 조용하지만 강렬한 끌림! 황소자리의 현실감각과 전갈자리의 직감이 만나 서로에게 없던 새로운 면을 발견하게 해주는 신비로운 관계입니다. "너 생각보다 깊은 사람이구나"를 자주 느끼게 되는 매력적인 조합입니다.
        
        💔 <b>조심할 궁합</b>
        
        🦁 <strong>사자자리</strong>: 황소자리가 "집에서 쉬자"하면 사자자리가 "밖에 나가서 사람들 만나자"하며 생활 패턴이 완전히 달라 피곤한 관계가 될 수 있습니다. 하지만 서로 타협하면 집순이와 파티 러버의 균형 잡힌 커플이 될 수도 있어요.
        
        🏺 <strong>물병자리</strong>: 황소자리가 "변화는 천천히"하면 물병자리가 "새로운 건 바로바로"하며 변화에 대한 태도 차이로 답답함을 느낄 수 있는 관계입니다.
        
        🤝 <b>우정으로는 최고</b>
        
        👥 <strong>쌍둥이자리</strong>: 연애로는 속도 차이가 있지만, 친구로는 완벽합니다. 쌍둥이자리의 재미있는 이야기를 황소자리가 편안하게 들어주는 힐링 친구 관계예요.`
    },
    3: {
        name: "쌍둥이자리 (Gemini)",
        ...
        element: "공기의 별자리",
        ruler: "수성",
        description: `소통의 신 헤르메스(수성)가 지배하는 쌍둥이자리. 그리스 신화의 쌍둥이 형제처럼 두 개의 다른 면을 가진 매력적인 별자리입니다.\n\n` +
  `바람의 속성을 지녀 어디로 날아갈지 예측 불가능한 자유로운 영혼들.\n\n` +
  `👥 **쌍둥이자리의 이중생활**\n\n` +
  `• 엘리베이터에 타면 어느새 옆 사람과 친구\n` +
  `• 카톡방에서는 항상 대화의 중심\n` +
  `• "그런데 말이야~"로 시작하는 흥미진진한 이야기꾼\n` +
  `• 모순 덩어리지만 그게 또 매력적\n\n` +
  `아침: "오늘부터 미니멀 라이프!"를 선언.\n\n` +
  `점심: 온라인 쇼핑몰에서 장바구니 채우는 중. 연애할 때는 매일 새로운 모습으로 상대방을 놀라게 하는 서프라이즈 마스터입니다.`,
        ...
        ♈ <strong>양자리</strong>: 에너지 폭발 듀오! 쌍둥이자리가 "이런 거 재미있을 것 같아!"하면 양자리가 "바로 해보자!"하며 아이디어를 즉석에서 실현시키는 실행력 만점 커플입니다. 둘 다 새로운 것을 좋아해서 지루할 틈이 없는 관계예요.
        
        🌟 <b>특별한 인연</b>
        
        🏹 <strong>사수자리</strong>: 모험과 지식의 조합! 쌍둥이자리가 "이거 신기하지 않아?"하면 사수자리가 "직접 가서 보자!"하며 책으로만 알던 걸 몸소 체험하게 해주는 역동적인 관계입니다. 서로의 호기심을 자극하며 끊임없이 성장하는 자극적인 커플이에요.
        ...
        🐟 <strong>물고기자리</strong>: 쌍둥이자리가 "논리적으로 생각해봐"하면 물고기자리가 "그냥 느낌적인 느낌이야"라며 서로 다른 언어를 쓰는 듯한 관계입니다. 소통 방식의 차이로 답답함을 느낄 수 있어요.
    ...
    4: {
        ...
        element: "물의 별자리",
        ruler: "달",
        description: `달의 신비로운 힘을 받는 게자리. 그리스 신화에서 헤라클레스와 싸우다 별이 된 용감한 게의 후예입니다.\n\n` +
  `밤하늘의 달처럼 변화무쌍하지만 언제나 따뜻한 빛으로 사람들을 위로하는 감성의 달인들.\n\n` +
  `🦀 **게자리의 츤데레 매뉴얼**\n\n` +
  `• "밥은 먹고 다녀?"라는 말로 하루를 시작\n` +
  `• "추우니까 옷 따뜻하게 입고 다녀"로 마무리\n` +
  `• 케어봇 같은 존재지만 표현은 서툴러요\n` +
  `• "나 별로 안 챙겨!"라면서 이미 챙길 리스트 작성 중\n\n` +
  `딱딱한 껍데기 안에 부드러운 마음을 숨긴 진정한 츤데레.\n\n` +
  `연애할 때는 "작년 이맘때 너가 그랬잖아"를 꺼내들어 상대방을 감동시키는 기억력의 마법사입니다.`,
        ...
        compatibility: `⭐ 최고의 궁합 ⭐
        
        🦂 전갈자리: 물의 깊은 교감! 게자리와 전갈자리는 말하지 않아도 서로의 마음을 아는 텔레파시 커플입니다. 게자리가 "요즘 힘들어"라고 하면 전갈자리가 벌써 치킨을 주문해놓은 찰떡궁합입니다. 둘 다 진짜 마음을 잘 안 보여주지만 서로에게만큼은 완전히 열어버리는 운명적인 관계입니다. 게자리의 케어 본능과 전갈자리의 보호 본능이 만나 서로를 아끼는 마음이 하늘을 찌르는 로맨틱한 커플이에요.
        
        🐟 <strong>물고기자리</strong>: 감성 충만 듀오! 게자리의 보살핌과 물고기자리의 순수함이 만나 "세상에 이렇게 따뜻한 사랑이 있을까?"를 보여주는 힐링 커플입니다. 함께 영화보며 울고, 비 오는 날 우산 하나 쓰고 걸어도 행복한 로맨틱 관계입니다. 게자리가 현실적인 케어를 하면 물고기자리가 감성적인 위로를 해주는 완벽한 역할분담을 보여줍니다.
        
        🐂 <strong>황소자리</strong>: 집순이들의 천국! 게자리와 황소자리는 "우리 집이 세상에서 제일 좋아"를 외치며 완벽한 홈 스위트 홈을 만드는 안정감 MAX 커플입니다. 황소자리가 맛있는 음식을 만들면 게자리가 예쁘게 상차림을 하고, 함께 소파에 앉아 드라마 보는 것이 최고의 데이트인 평화로운 관계입니다.
        
        🌟 <b>특별한 인연</b>
        
        🐐 <strong>염소자리</strong>: 감성과 이성의 완벽한 조화! 게자리의 따뜻한 감정과 염소자리의 냉철한 이성이 만나 "네가 없으면 안 될 것 같아"를 느끼게 하는 보완 관계입니다. 게자리가 "마음이 힘들어"하면 염소자리가 "이렇게 하면 해결될 것 같아"라며 실질적인 도움을 주는 든든한 파트너십입니다. 서로 없던 부분을 채워주며 함께 성장하는 관계예요.
        
        🌾 <strong>처녀자리</strong>: 케어의 달인들! 게자리의 감정적 케어와 처녀자리의 실용적 케어가 만나 "우리 서로 완벽하게 챙겨주네"를 실감하는 세심한 관계입니다. 처녀자리가 건강 챙기면 게자리가 마음 챙기고, 서로에게 없어서는 안 될 소중한 존재가 되어주는 따뜻한 커플입니다.
        
        💔 <b>조심할 궁합</b>
        
        ♈ <strong>양자리</strong>: 게자리가 "천천히 생각해보자"하면 양자리가 "왜 이렇게 답답해?"하며 속도 차이로 지치는 관계입니다. 게자리의 신중함과 양자리의 성급함이 "너무 느려" vs "너무 빨라"로 충돌할 수 있어요. 하지만 양자리가 조금만 기다려주고 게자리가 조금만 빨라지면 의외로 좋은 조합이 될 수 있습니다.
        
        ⚖️ <strong>천칭자리</strong>: 게자리의 직설적 감정 표현과 천칭자리의 외교적 화법이 "진심이 뭐야?"로 이어지는 답답한 관계가 될 수 있습니다. 게자리가 "솔직하게 말해줘"하면 천칭자리가 "둘 다 이해돼"라며 명확한 답변을 피하는 패턴이 반복될 수 있어요.
        
        🏺 <strong>물병자리</strong>: 게자리의 감정 중시와 물병자리의 이성 중시가 "너무 감정적이야" vs "너무 차가워"로 충돌하는 관계입니다. 서로 다른 소통 방식으로 인해 오해가 생기기 쉬운 조합이에요.
        
        🤝 <b>우정으로는 최고</b>
        
        👥 <strong>쌍둥이자리</strong>: 연애로는 속도 차이가 있지만, 친구로는 완벽합니다! 쌍둥이자리의 재미있는 소식들을 게자리가 따뜻하게 들어주고, 게자리가 힘들 때는 쌍둥이자리가 웃긴 이야기로 기분을 풀어주는 힐링 친구 관계예요.`
    },
    5: {
        name: "사자자리 (Leo)",
        ...
        element: "불의 별자리",
        ruler: "태양",
        description: `태양의 찬란한 빛을 받아 태어난 사자자리. 그리스 신화의 네메아 사자처럼 강인하고 당당한 왕의 별자리입니다.\n\n` +
  `헤라클레스도 감탄한 그 위엄과 카리스마로 어디서든 자연스럽게 중심이 되는 타고난 리더들.\n\n` +
  `🦁 **사자자리의 메인 캐릭터 라이프**\n\n` +
  `• 셀카 실력은 프로급 수준\n` +
  `• SNS 피드는 언제나 완벽\n` +
  `• "오늘 옷 예쁘네!" 한마디에 온종일 기분 좋아지는 칭찬 중독자\n` +
  `• 시선 집중은 기본, 따뜻한 칭찬도 아끼지 않아요\n\n` +
  `방에 들어서는 순간 모든 시선이 자동 집중.\n\n` +
  `하지만 다른 사람도 아낌없이 칭찬해주는 따뜻한 마음의 소유자. 연애할 때는 상대방을 왕족처럼 모시지만, 자신도 똑같은 대우를 원하는 상호존중주의자입니다.`,
        ...
        ♈ <strong>양자리</strong>: 불의 왕과 개척자! 사자자리와 양자리는 마치 액션 영화의 주인공들처럼 "우리가 최고야!"를 외치며 세상을 정복하러 다니는 파워 커플입니다. 서로의 자신감을 북돋아주고, "당신 정말 멋있어!"를 아끼지 않는 칭찬 부자들입니다. 데이트할 때도 "오늘 뭐 할까?"라는 질문에 동시에 "신나는 거!"라고 답하는 찰떡궁합이에요.
        
        🏹 <strong>사수자리</strong>: 모험의 로열 커플! 사자자리의 리더십과 사수자리의 모험심이 만나 "인생은 축제야!"를 실천하는 활기찬 관계입니다. 함께 있으면 매일이 파티이고, 여행 가면 현지인들과도 금방 친해지는 인기 만점 듀오입니다. 사자자리가 "우리 사진 찍자!"하면 사수자리가 "저기 배경 예쁜 데서!"라며 완벽한 여행 콘텐츠를 만들어내는 SNS 스타 커플이기도 해요.
        
        👥 <strong>쌍둥이자리</strong>: 에너지와 재치의 조합! 사자자리의 당당함과 쌍둥이자리의 위트가 만나 어디서든 주목받는 엔터테이너 커플이 됩니다. 사자자리가 "내가 주인공!"이면 쌍둥이자리가 "내가 해설자!"가 되어 완벽한 콤비를 이루는 재미있는 관계입니다.
        
        🌟 <b>특별한 인연</b>
        
        🏺 <strong>물병자리</strong>: 정반대의 매력! 사자자리의 화려함과 물병자리의 독특함이 만나 "너는 정말 특별해"를 느끼게 하는 신선한 관계입니다. 사자자리가 스포트라이트를 받으면 물병자리가 뒤에서 "잘했어!"하며 조용히 응원해주는 조화로운 커플입니다. 서로 다른 매력으로 끌리면서도 존중하는 성숙한 관계예요.
        
        ⚖️ <strong>천칭자리</strong>: 화려함과 우아함의 만남! 사자자리의 당당한 카리스마와 천칭자리의 세련된 매력이 만나 "우리 정말 잘 어울리지 않아?"를 보여주는 비주얼 커플입니다. 함께 있으면 주변 사람들이 "와, 멋있다"를 연발하는 스타일리시한 관계입니다.
        
        💔 <b>조심할 궁합</b>
        
        🐂 <strong>황소자리</strong>: 사자자리가 "나가서 놀자!"하면 황소자리가 "집이 편해"하며 생활 패턴 차이로 지치는 관계입니다. 사자자리의 활동적인 성향과 황소자리의 안정 추구가 "너무 바빠" vs "너무 집에만 있어"로 충돌할 수 있어요. 하지만 서로 타협하면 집순이와 파티 러버의 균형 잡힌 커플이 될 수도 있습니다.
        
        🦂 <strong>전갈자리</strong>: 둘 다 강한 개성을 가져 "내가 주인공이야!" vs "나도 만만치 않아!"로 권력 다툼이 벌어질 수 있는 관계입니다. 사자자리의 오픈된 카리스마와 전갈자리의 은밀한 강함이 충돌해 서로를 이해하기 어려울 수 있어요.
        
        🌾 <strong>처녀자리</strong>: 사자자리가 "대충 해도 돼"하면 처녀자리가 "완벽하게 하자"하며 접근 방식의 차이로 답답해하는 관계입니다. 사자자리의 즉흥성과 처녀자리의 신중함이 속도 차이를 만들 수 있어요.
        
        🤝 <b>우정으로는 최고</b>
        
        🦀 <strong>게자리</strong>: 연애로는 속도와 성향 차이가 있지만, 친구로는 환상적입니다! 사자자리의 밝은 에너지가 게자리에게 활력을 주고, 게자리의 따뜻한 케어가 사자자리에게 안정감을 주는 서로를 보완하는 좋은 친구 관계예요.`
    },
    6: {
        name: "처녀자리 (Virgo)",
        ...
        element: "흙의 별자리",
        ruler: "수성",
        description: `수확의 여신 데메테르가 지키는 처녀자리. 그리스 신화 아스트라이아 여신의 후예로 순수함과 지혜를 상징합니다.\n\n` +
  `완벽함을 추구하되 따뜻한 마음으로 세상을 더 나은 곳으로 만들어가는 실용적인 완벽주의자들.\n\n` +
  `🌾 **처녀자리의 디테일 장인 정신**\n\n` +
  `• "어 너 오늘 뭔가 달라 보여" - 정확히 포인트 찾는 관찰력\n` +
  `• 99점 받고도 "1점이 어디서?" 고민하는 완벽주의\n` +
  `• 건강 관리 전문가지만 스트레스 받으면 치킨 주문하는 반전\n` +
  `• "이렇게 하면 더 편할 텐데" 진심 조언하는 케어 전문가\n\n` +
  `연애할 때는 상대방을 위한 세심한 배려가 특기.\n\n` +
  `그 꼼꼼함 덕분에 사랑받는 소중한 존재들입니다.`,
        ...
        🐂 <strong>황소자리</strong>: 현실적인 완벽 듀오! 처녀자리와 황소자리는 "우리 집 정말 완벽하지 않아?"를 실현시키는 실용성의 달인들입니다. 처녀자리가 계획을 세우면 황소자리가 착실히 실행하여 꿈의 라이프스타일을 만들어가는 안정감 MAX 커플입니다.
        
        🐐 <strong>염소자리</strong>: 성공의 파트너십! 처녀자리의 완벽함과 염소자리의 야망이 만나 "우리가 하면 무조건 성공해"를 보여주는 워킹 커플입니다. 함께 목표를 세우고 달성하는 것이 취미이며, 주변에서 부러워하는 모범적인 관계입니다.
        
        🌟 <strong>특별한 인연</strong>
        🐟 <strong>물고기자리</strong>: 이성과 감성의 완벽한 조화! 처녀자리가 "현실적으로 생각해봐"하면 물고기자리가 "마음은 어떻게 할 거야?"라며 서로 없던 부분을 채워주는 보완 관계입니다. 처녀자리는 더 따뜻해지고 물고기자리는 더 현실적이 되는 성장하는 커플입니다.
        
        💔 <strong>주의할 궁합</strong>
        👥 <strong>쌍둥이자리</strong>: 처녀자리가 "체계적으로 하자"하면 쌍둥이자리가 "그때그때 달라요"하며 접근 방식의 차이로 답답해하는 관계입니다.
        
        🏹 <strong>사수자리</strong>: 처녀자리의 디테일 챙기기와 사수자리의 대충대충이 만나 "너무 꼼꼼해" vs "너무 대충이야"를 주고받는 피곤한 관계가 될 수 있습니다.`
    },
    7: {
        name: "천칭자리 (Libra)",
        ...
        element: "공기의 별자리",
        ruler: "금성",
        description: `아름다움과 조화의 여신 비너스(금성)의 축복을 받은 천칭자리. 그리스 신화의 정의의 여신 디케처럼 공정하고 아름다운 것들을 사랑합니다.\n\n` +
  `신중하게 저울 양편의 무게를 재듯 모든 것에 진정한 균형을 찾으려 노력하는 예술가들.\n\n` +
  `⚖️ **천칭자리의 선택 장애 실화**\n\n` +
  `• 레스토랑에서 메뉴판 30분 동안 들여다보는 선택의 마에스트로\n` +
  `• "둘 다 맞아, 둘 다 이해해"로 모든 갈등 중재하는 타고난 외교관\n` +
  `• 인스타그램은 예술 갤러리 수준의 미적 감각 소유자\n` +
  `• '인테리어 예쁘지? 음식 맛있지?'가 카페 고르는 기준\n\n` +
  `완벽한 미학주의자의 일상.\n\n` +
  `연애할 때는 "우리 항상 공평하게 하자"를 모토로 살지만, 속으로는 상대방의 진심을 계산하고 있는 로맨틱 회계사들입니다.`,
        ...
        👥 <strong>쌍둥이자리</strong>: 지적인 소울메이트! 천칭자리와 쌍둥이자리는 "우리 진짜 통한다"를 매일 실감하는 커뮤니케이션의 달인들입니다. 카페에서 만나면 3시간이 3분처럼 느껴지고, 서로의 농담에 배꼽 잡고 웃는 유쾌한 관계입니다.
        
        🏺 <strong>물병자리</strong>: 자유로운 예술가 듀오! 천칭자리의 미적 감각과 물병자리의 창의성이 만나 "우리 독특하면서도 멋있지 않아?"를 보여주는 스타일리시한 커플입니다. 서로의 개성을 존중하며 "같이 있지만 따로" 철학을 실천하는 쿨한 관계입니다.
        
        🌟 <strong>특별한 인연</strong>
        ♈ <strong>양자리</strong>: 정반대의 매력! 천칭자리가 "생각해보자"하면 양자리가 "일단 해보자"하며 서로 없던 용기와 신중함을 배우게 해주는 성장 관계입니다. 천칭자리는 더 결단력 있게, 양자리는 더 신중하게 되는 WIN-WIN 커플입니다.
        
        💔 <strong>주의할 궁합</strong>
        🦀 <strong>게자리</strong>: 천칭자리의 사교성과 게자리의 집돌이 성향이 "나가자" vs "집에 있자"로 충돌하는 관계입니다.
        
        🐐 <strong>염소자리</strong>: 천칭자리가 "예쁘면 되지"하면 염소자리가 "실용성은?"하며 가치관 차이로 답답해하는 관계가 될 수 있습니다.`
    },
    8: {
        name: "전갈자리 (Scorpio)",
        ...
        element: "물의 별자리",
        ruler: "명왕성, 화성",
        description: `지하세계의 왕 하데스(명왕성)와 전쟁의 신 아레스(화성)의 이중 지배를 받는 전갈자리. 그리스 신화의 불사조 피닉스처럼 죽음과 재생을 동시에 품은 신비로운 별자리입니다.\n\n` +
  `강렬한 에너지로 모든 것을 변화시키는 변화의 마법사들.\n\n` +
  `🦂 **전갈자리의 비밀 운영체제**\n\n` +
  `• SNS 프로필은 비공개, 카톡 상태메시지도 '의미심장'\n` +
  `• 남의 비밀은 귀신같이 알아내는 FBI 수준의 직관력\n` +
  `• 차가운 얼음 바깥, 뜨거운 용암 내면을 가진 인간 버전 빙산\n` +
  `• 첫만남: '왜 이렇게 차가울까?' → '내 사람' 인증 후: 의리의 끝판왕\n\n` +
  `한번 마음을 열면 자신의 전부를 바칠 준비 완료.\n\n` +
  `연애할 때는 소유욕이 강하지만 사랑에 빠지면 정말 진심으로 바치는 대박 로맨티스트입니다.`,
        ...
        🦀 <strong>게자리</strong>: 물의 깊은 유대감! 전갈자리와 게자리는 말하지 않아도 서로의 마음을 읽는 텔레파시 커플입니다. "요즘 어때?"라는 말 한마디에 담긴 진심을 알아차리고, 힘들 때 곁에서 묵묵히 지켜주는 평생의 동반자 관계입니다.
        
        🐟 <strong>물고기자리</strong>: 감정의 오케스트라! 전갈자리의 강렬함과 물고기자리의 순수함이 만나 "이런 사랑이 존재할 줄 몰랐어"를 느끼게 하는 로맨틱한 관계입니다. 서로의 감정을 100% 이해하고 공감하는 영혼의 짝꿍입니다.
        
        🌟 <strong>특별한 인연</strong>
        🐂 <strong>황소자리</strong>: 강함과 안정의 만남! 전갈자리의 깊이와 황소자리의 현실감각이 만나 "너와 함께라면 무엇도 두렵지 않아"를 실감하는 든든한 관계입니다. 전갈자리가 열정을 쏟으면 황소자리가 현실적으로 받쳐주는 완벽한 팀워크를 보여줍니다.
        
        💔 <strong>주의할 궁합</strong>
        🦁 <strong>사자자리</strong>: 둘 다 강한 개성으로 "내가 주도권을 잡을 거야"를 두고 은근한 신경전이 벌어질 수 있는 관계입니다.
        
        🏺 <strong>물병자리</strong>: 전갈자리의 집착과 물병자리의 자유로움이 "너무 매달려" vs "너무 차가워"로 충돌하는 관계입니다.`
    },
    9: {
        ...
        element: "불의 별자리",
        ruler: "목성",
        description: `더 높은 차원을 추구하는 목성의 축복을 받은 사수자리. 그리스 신화의 현명한 켄타우로스처럼 끝없이 새로운 지평선을 향해 달려가는 자유로운 탐험가들입니다.\n\n` +
  `미지의 것을 두려워하지 않고 오히려 반기는 철학자이자 모험가들.\n\n` +
  `🏹 **사수자리의 즉흥 모험신**\n\n` +
  `• 금요일 오후 "내일 아침 비행기 있어, 가니?" → "그래, 가자!"\n` +
  `• 여행지에서 현지 사람들과 5분 만에 친구 되는 사교력\n` +
  `• "이거 대박 신기하지 않아?" 연속 감탄하는 호기심 폭발\n` +
  `• 계획 틀어져도 "또 다른 재미있는 길이 열린 거네!" 긍정왕\n\n` +
  `실패도 새로운 모험의 시작으로 받아들이는 인생 여행 라이브 스트리머.\n\n` +
  `연애할 때는 "우리는 각자 자유롭게 살면서 함께 하는 거야"라는 바람처럼 자유로운 연인입니다.`,
        ...
        ♈ <strong>양자리</strong>: 자유로운 모험가들! 사수자리와 양자리는 "인생은 한 번뿐이야!"를 외치며 세상을 탐험하는 활력 넘치는 커플입니다. 서로의 도전 정신을 응원하고, "너라면 뭐든 할 수 있어!"를 진심으로 믿어주는 든든한 동반자입니다.
        
        🦁 <strong>사자자리</strong>: 인생의 축제 메이커들! 사수자리의 낙천적 에너지와 사자자리의 화려함이 만나 "매일이 파티야!"를 실현시키는 핫한 커플입니다. 함께 여행 가면 현지에서 벌써 친구가 생기고, 어디서든 주목받는 인기 만점 듀오입니다.
        
        🌟 <strong>특별한 인연</strong>
        👥 <strong>쌍둥이자리</strong>: 지식과 모험의 만남! 사수자리가 "직접 가서 보자!"하면 쌍둥이자리가 "그거 알고 가자!"하며 완벽한 여행 계획을 세워주는 콤비입니다. 서로의 호기심을 자극하며 끊임없이 성장하는 자극적인 관계입니다.
        
        💔 <strong>주의할 궁합</strong>
        🌾 <strong>처녀자리</strong>: 사수자리가 "즉흥적으로 가자"하면 처녀자리가 "계획부터 세우자"하며 접근 방식의 차이로 답답해하는 관계입니다.
        
        🐟 <strong>물고기자리</strong>: 사수자리의 직설적 화법과 물고기자리의 감수성이 "너무 무심해" vs "너무 예민해"로 상처 주고받는 관계가 될 수 있습니다.`
    },
    10: {
        name: "염소자리 (Capricorn)",
        ...
        element: "흙의 별자리",
        ruler: "토성",
        description: `시간과 인내의 신 크로노스(토성)의 엄격한 가르침을 받은 염소자리. 그리스 신화의 염소처럼 한 걸음씩 착실하게 높은 곳을 향해 오르는 야심찬 성취의 달인들입니다.\n\n` +
  `느리지만 확실하게, 전통을 소중히 여기면서도 현실적인 변화를 만들어가는 지혜로운 리더들.\n\n` +
  `🐐 **염소자리의 성공 공식**\n\n` +
  `• 휴가 중에도 노트북 챙기는 '잠깐만, 이것만 할게' 일 중독자\n` +
  `• 20대부터 차근차근 인생 계획을 세우는 현실주의자\n` +
  `• 술 한잔 들어가면 '사실 나도 그러고 싶다' 고백하는 귀여운 면\n` +
  `• 명절에는 '내가 어릴 때는 말이야...' 스토리 필수 코스\n\n` +
  `전통을 지키는 든든한 기둥 같은 존재.\n\n` +
  `연애할 때는 '은퇴 후 어디서 살지'까지 계획하지만, 정작 중요한 건 말보다 행동으로 보여주는 진정한 로맨티스트입니다.`,
        ...
        🐂 <strong>황소자리</strong>: 안정감의 대명사! 염소자리와 황소자리는 "우리 진짜 잘살 것 같아"를 현실로 만드는 실속형 커플입니다. 염소자리가 야심차게 계획하면 황소자리가 차근차근 실현해나가며, 주변에서 부러워하는 모범적인 가정을 꾸려갑니다.
        
        🌾 <strong>처녀자리</strong>: 완벽한 성공 공식! 염소자리의 목표 지향성과 처녀자리의 완벽주의가 만나 "우리가 하면 반드시 성공한다"를 증명하는 파워 커플입니다. 함께 사업을 해도, 가정을 꾸려도 모든 것이 체계적이고 완벽한 황금 조합입니다.
        
        🌟 <strong>특별한 인연</strong>
        🦀 <strong>게자리</strong>: 이성과 감성의 완벽한 밸런스! 염소자리가 "현실적으로 생각해"하면 게자리가 "마음은 어때?"라며 서로 없던 따뜻함과 계획성을 배우게 해주는 성장 관계입니다. 염소자리는 더 감성적으로, 게자리는 더 현실적으로 되는 WIN-WIN 커플입니다.
        
        💔 <strong>주의할 궁합</strong>
        ⚖️ <strong>천칭자리</strong>: 염소자리가 "실용적이야"하면 천칭자리가 "아름다워야지"하며 가치관 차이로 충돌하는 관계입니다.
        
        ♈ <strong>양자리</strong>: 염소자리의 신중함과 양자리의 성급함이 "천천히 하자" vs "빨리 하자"로 속도 차이에 지치는 관계가 될 수 있습니다.`
    },
    11: {
        name: "물병자리 (Aquarius)",
        ...
        element: "공기의 별자리",
        ruler: "천왕성, 토성",
        description: `미래와 혁신의 신 우라노스(천왕성)의 영향을 받는 물병자리. 그리스 신화의 물병을 든 소년처럼 전 인류에게 새로운 시대의 지혜를 전하는 미래지향적인 인도주의자들입니다.\n\n` +
  `예측 불가능한 독창성으로 세상을 더 나은 곳으로 변화시키는 꿈의 설계자들.\n\n` +
  `🏺 **물병자리의 반전 문화**\n\n` +
  `• '남들이 오른쪽으로 가면 왼쪽으로' 당연하게 생각하는 반골 기질\n` +
  `• 10년 후를 예측하는 미래학자이자 신기술 얼리 어답터\n` +
  `• 친구는 많지만 '내 진짜 친구'는 한둘인 까다로운 사교성\n` +
  `• '나도 내가 누군지 모르겠어' → 남들은 정확히 분석해주는 신기함\n\n` +
  `자신은 모르겠다면서 남들은 척척 분석하는 신기한 능력 보유.\n\n` +
  `연애할 때는 '사랑해'보다 '너 진짜 특별하다'가 더 진심 어린 표현인 독특한 로맨티스트입니다.`,
        ...
        👥 <strong>쌍둥이자리</strong>: 아이디어 파트너들! 물병자리와 쌍둥이자리는 "이런 건 어때?"로 시작해서 세상에 없던 창의적인 프로젝트를 만들어내는 혁신 듀오입니다. 서로의 독특함을 이해하고 존중하며, "너는 정말 특별해"를 자주 느끼는 자극적인 관계입니다.
        
        ⚖️ <strong>천칭자리</strong>: 쿨하고 세련된 커플! 물병자리의 독창성과 천칭자리의 균형감이 만나 "우리 진짜 멋있지 않아?"를 보여주는 스타일리시한 관계입니다. 서로의 공간을 존중하며 "같이 있지만 따로"의 이상적인 관계를 실현합니다.
        
        🌟 <strong>특별한 인연</strong>
        🦁 <strong>사자자리</strong>: 정반대의 끌림! 물병자리의 쿨함과 사자자리의 따뜻함이 만나 서로에게 없던 새로운 면을 발견하게 해주는 신선한 관계입니다. 사자자리는 더 개성 있게, 물병자리는 더 따뜻하게 변화하는 성장하는 커플입니다.
        
        💔 <strong>주의할 궁합</strong>
        🦂 <strong>전갈자리</strong>: 물병자리의 자유로움과 전갈자리의 집착이 "너무 차가워" vs "너무 매달려"로 충돌하는 관계입니다.
        
        🐂 <strong>황소자리</strong>: 물병자리가 "변화가 필요해"하면 황소자리가 "지금도 좋은데"하며 변화에 대한 태도 차이로 답답해하는 관계가 될 수 있습니다.`
    },
    12: {
        name: "물고기자리 (Pisces)",
        ...
        element: "물의 별자리",
        ruler: "해왕성, 목성",
        description: `마지막 별자리로서 12신의 지혜를 모두 가지고 태어난 물고기자리. 바다의 신 포세이돈(해왕성)의 축복을 받았습니다.\n\n` +
  `그리스 신화의 아프로디테처럼 사랑과 미의 아름다움을 아는 예술가들이며, 동시에 모든 상처를 치유하는 휴머니스트들.\n\n` +
  `🐟 **물고기자리의 감성 블로그**\n\n` +
  `• 지하철에서 '오늘 날씨가 첫사랑 떠올리던 날 같네' 감성 스토리 제작\n` +
  `• 드라마 주인공이 울면 같이 울고, 친구 연애에 더 아파하는 공감 능력\n` +
  `• "돈은 쓸 수 있을 때!" 충동구매 후 월말 라면 먹는 반전 매력\n` +
  `• 비 오는 날엔 '뜨거운 차와 감성적인 음악' 자동 연상\n\n` +
  `모든 순간을 로맨틱하게 만드는 타고난 감성.\n\n` +
  `연애할 때는 '이럴 때 이런 노래 들으면 어떨까?'라며 상대방의 일상을 영화로 만드는 로맨스 전문가입니다.`,
        ...
        🦀 <strong>게자리</strong>: 감성의 소울메이트! 물고기자리와 게자리는 "우리 진짜 통한다"를 매순간 느끼는 힐링 커플입니다. 말하지 않아도 서로의 기분을 알아채고, 힘들 때는 묵묵히 안아주고, 기쁠 때는 함께 울어주는 감정 공유의 달인들입니다. 게자리가 현실적인 케어를 해주면 물고기자리가 감성적인 위로를 해주는 완벽한 역할분담을 보여주는 따뜻한 관계예요.
        
        🦂 <strong>전갈자리</strong>: 깊은 영혼의 만남! 물고기자리의 순수함과 전갈자리의 깊이가 만나 "이런 사랑이 존재했구나"를 깨닫게 해주는 운명적인 관계입니다. 서로의 감정을 100% 이해하고 받아들이며, 세상 그 누구보다 특별한 존재가 되어주는 영혼의 짝꿍입니다. 전갈자리의 보호 본능과 물고기자리의 순수함이 만나 서로에게 없어서는 안 될 존재가 되는 깊은 사랑을 나누는 커플이에요.
        
        🐂 <strong>황소자리</strong>: 꿈과 현실의 아름다운 조화! 물고기자리의 상상력과 황소자리의 현실감각이 만나 "꿈을 현실로 만드는" 마법 같은 관계입니다. 물고기자리가 "이런 게 있으면 좋겠어"하면 황소자리가 "그럼 이렇게 해보자"라며 실제로 만들어주는 든든한 파트너십을 보여줍니다.
        
        🌟 <b>특별한 인연</b>
        
        🌾 <strong>처녀자리</strong>: 꿈과 현실의 완벽한 조화! 물고기자리가 "이런 꿈이 있어"하면 처녀자리가 "이렇게 하면 될 것 같아"라며 현실적인 방법을 제시해주는 보완 관계입니다. 물고기자리는 더 실용적으로, 처녀자리는 더 감성적으로 변화하는 성장하는 커플입니다. 서로 정반대 같지만 실제로는 완벽하게 맞아떨어지는 퍼즐 조각 같은 관계예요.
        
        🐐 <strong>염소자리</strong>: 감성과 현실의 만남! 물고기자리의 로맨틱함과 염소자리의 안정감이 만나 "사랑도 있고 미래도 있고"를 실현시키는 균형 잡힌 관계입니다. 염소자리가 현실적인 계획을 세우면 물고기자리가 그 안에 따뜻한 감성을 불어넣어주는 아름다운 팀워크를 보여줍니다.
        
        💔 <b>조심할 궁합</b>
        
        👥 <strong>쌍둥이자리</strong>: 물고기자리가 "느낌적으로"하면 쌍둥이자리가 "논리적으로 생각해봐"하며 소통 방식의 차이로 답답해하는 관계입니다. 물고기자리의 직관적 사고와 쌍둥이자리의 논리적 사고가 "왜 이해 못 해?" vs "왜 설명 못 해?"로 충돌할 수 있어요. 하지만 서로 다른 방식을 인정하면 오히려 새로운 관점을 배울 수 있는 관계이기도 해요.
        
        🏹 <strong>사수자리</strong>: 물고기자리의 섬세함과 사수자리의 직설적 화법이 "너무 상처받아" vs "그렇게 예민하게"로 상처 주고받는 관계가 될 수 있습니다. 사수자리의 솔직함이 물고기자리에게는 너무 날카롭게 느껴질 수 있어요.
        
        🏺 <strong>물병자리</strong>: 물고기자리가 "감정적으로 생각해"하면 물병자리가 "객관적으로 봐야지"하며 접근 방식의 차이로 서로를 이해하기 어려운 관계입니다. 물고기자리의 감정 중심적 사고와 물병자리의 합리적 사고가 다른 언어를 쓰는 것처럼 느껴질 수 있어요.
        
        🤝 <b>우정으로는 최고</b>
        
        ⚖️ <strong>천칭자리</strong>: 연애로는 결정력과 우유부단함의 차이가 있지만, 친구로는 완벽합니다! 물고기자리의 순수한 감성을 천칭자리가 아름답게 승화시켜주고, 천칭자리가 고민할 때 물고기자리가 따뜻하게 공감해주는 서로를 힐링시켜주는 좋은 친구 관계예요.`
    }
};
