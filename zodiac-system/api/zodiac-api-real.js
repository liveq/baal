    ...
    async getCompatibility(zodiac1Id, zodiac2Id) {
        ...
        if (dbCompatData && dbCompatData.compat_message) {
            ...
            return {
                ...
                description: removeHtmlTags(this.ensureMessageQuality(dbCompatData.compat_message)),
                advice: removeHtmlTags(this.ensureMessageQuality(dbCompatData.advice || "서로를 이해하고 배려하는 마음이 중요합니다.")),
                source: 'database-compat-message'
            };
        }
        
        // 2순위: 기존 JSON 데이터 사용 (폴백)
        if (!this.fortuneData) {
            await this.loadFortuneData();
        }
        ...
        // 궁합 설명 결정 - compatibility-data.json의 compat_message만 사용
        let displayText = null;

        // zodiacDescriptions 사용 중단 - compatibility-data.json 데이터만 사용
        console.log('🎯 compatibility-data.json 메시지 사용');
        try {
            // JSON 데이터의 compat_message 사용 (올바른 필드)
            if (jsonData?.compat_message) {
                displayText = this.ensureMessageQuality(jsonData.compat_message);
                console.log('✅ JSON compat_message 사용 (품질 검증 완료):', compatKey);
            } else {
                // 기본 메시지
        ...
        if (jsonData) {
            ...
            return {
                ...
                description: removeHtmlTags(displayText),
                advice: removeHtmlTags(this.ensureMessageQuality(jsonData.advice || "서로를 이해하고 배려하는 마음이 중요합니다.")),
                source: 'json-compatibility-data'
            };
        } else {
            ...
            return {
                ...fallbackData,
                description: removeHtmlTags(displayText),
                advice: removeHtmlTags(this.ensureMessageQuality(fallbackData.advice)),
                source: 'generated-filtered'
            };
        }
    }
