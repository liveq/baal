/**
 * 별자리 운세 API - 실제 데이터 + 역사적 인물 + 메시지 필터 통합 버전
...
class ZodiacAPIReal {
    ...
    generateEnhancedMessage(figure, category, baseMessage) {
        ...
        if (achievement) {
            ...
            enhancedMessage = `${figure.name}이 ${achievementName}를 만들어낸 것처럼, ${trait}`;
        } else {
    ...
    formatWithConcreteExample(figure, trait) {
        ...
        // 대표 업적 활용
            ...
            formattedMessage = `${figure.name}이 ${achievement}를 창조했듯이, ${trait}`;
        }
        else {
