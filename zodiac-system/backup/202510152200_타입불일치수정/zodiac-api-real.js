...
class ZodiacAPIReal {
    ...
    async loadCompatibilityData() {
        console.log('🔄 loadCompatibilityData() 시작');
        console.log('📁 시도할 경로: api/compatibility-data.json');

        try {
            console.log('🌐 fetch 요청 시작...');
            const response = await fetch('api/compatibility-data.json');
            ...
            console.log('✅ Compatibility data loaded successfully!');
            console.log('📋 데이터 구조:', {
                총조합수: Object.keys(this.compatibilityData).length,
                샘플키: Object.keys(this.compatibilityData)[0],
                샘플데이터: this.compatibilityData[Object.keys(this.compatibilityData)[0]]
            });

        } catch (error) {
            console.error('❌ compatibility-data.json 로드 실패:', error);
            console.error('에러 상세:', {
                name: error.name,
            ...
            this.compatibilityData = null;
            console.log('📦 compatibilityData를 null로 설정 (폴백 시스템 사용)');
        }
    }
    ...
    async getDailyFortune(zodiacId) {
        ...
        if (fortuneData) {
            ...
            // 양자리만 구체적 조언 적용 - 카테고리별 다른 인물 사용
            const enhancedFortunes = {};
            if (fortuneData.fortunes) {
                for (const [category, message] of Object.entries(fortuneData.fortunes)) {
                    if (zodiacId === 1) {
                        // 양자리: 카테고리별 다른 인물 선택하여 구체적 업적 포함 메시지 생성
                        const categoryFigure = this.selectHistoricalFigure(zodiacId, today, category);
                        if (categoryFigure) {
                            console.log(`♈ 양자리 ${category} 카테고리 → ${categoryFigure.name}`);
                            enhancedFortunes[category] = this.generateAriesSpecificMessage(categoryFigure, category, message);
                        } else {
                            console.log(`♈ 양자리 ${category} 카테고리: 인물 없음, 기본 메시지 사용`);
                            enhancedFortunes[category] = this.ensureMessageQuality(message);
                        }
                    } else {
                        // 다른 별자리: 기존 메시지 그대로 사용
                        enhancedFortunes[category] = this.ensureMessageQuality(message);
                    }
                }
            }
            ...
            // 양자리 폴백 안전장치 - 카테고리별 다른 인물 사용
            const safeOverall = zodiacId === 1 ? 
                (() => {
                    const overallFigure = this.selectHistoricalFigure(zodiacId, today, 'overall');
                    return overallFigure ? 
                        this.generateAriesSpecificMessage(overallFigure, 'overall', fortuneData.overall) :
                        this.ensureMessageQuality(fortuneData.overall);
                })() :
                this.ensureMessageQuality(fortuneData.overall);
                
            const safeAdvice = zodiacId === 1 ? 
                (() => {
                    const adviceFigure = this.selectHistoricalFigure(zodiacId, today, 'advice');
                    return adviceFigure ? 
                        this.generateAriesSpecificMessage(adviceFigure, 'advice', fortuneData.advice) :
     ...
     */
    normalizeCompatibilityData(data) {
        if (!data) return null;
        ...
        // 총 점수 추출 (다양한 필드명 지원)
        const totalScore = data.overall_score || data.totalScore || data.score || 75;

        // 개별 점수 추출 (중첩 구조 또는 flat 구조 모두 지원)
        let scores = {
            love: 75,
            friendship: 75,
            work: 75
        ...
        if (data.scores && typeof data.scores === 'object') {
            // fortune-data.json 형식: { scores: { love: 53, friendship: 78, work: 64 } }
            scores = {
                love: data.scores.love || 75,
                friendship: data.scores.friendship || 75,
                work: data.scores.work || 75
            };
        } else {
            // compatibility-data.json 형식: { love_score: 53, friendship_score: 78, work_score: 64 }
            scores = {
                love: data.love_score || data.love || 75,
                friendship: data.friendship_score || data.friendship || 75,
                work: data.work_score || data.work || 75
            };
        }

        // 설명 텍스트 추출
        const description = data.compat_message || data.message || data.description || "";
        const advice = data.advice || "서로를 이해하고 배려하는 마음이 중요합니다.";

        return {
            totalScore,
            scores,
            description: removeHtmlTags(description),
            advice: removeHtmlTags(advice)
        };
    }

    /**
     * 별자리 궁합 확인 (데이터베이스 compat_message 우선 사용)
     */
    async getCompatibility(zodiac1Id, zodiac2Id) {
        ...
        if (dbCompatData) {
            console.log('✅ compatibility-data.json 사용');
            const normalized = this.normalizeCompatibilityData(dbCompatData);
        ...
        // 2순위: fortune-data.json에서 데이터 가져오기 (폴백)
        console.log('⚠️ compatibility-data.json에서 데이터 없음, fortune-data.json 시도');
        if (!this.fortuneData) {
            await this.loadFortuneData();
        }
        ...
        if (this.fortuneData.compatibility && this.fortuneData.compatibility[compatKey]) {
            const jsonData = this.fortuneData.compatibility[compatKey];
            console.log('✅ fortune-data.json에서 데이터 발견:', compatKey);

            const normalized = this.normalizeCompatibilityData(jsonData);
            if (normalized) {
                return {
                    zodiac1Id: zodiac1Id,
                    zodiac2Id: zodiac2Id,
                    ...
                    description: normalized.description,
                    advice: normalized.advice,
                    source: 'fortune-data-json'
                };
            }
        }

        // 3순위: 기본 데이터 생성
        console.log('🔧 모든 데이터 소스 실패, 기본 데이터 생성');
        const fallbackData = await this.generateCompatibility(zodiac1Id, zodiac2Id);
        const normalized = this.normalizeCompatibilityData(fallbackData);
    ...
    /**
     * 데이터베이스에서 궁합 데이터 가져오기 (이미 로드된 compatibilityData 사용)
     */
    async getCompatibilityFromDatabase(zodiac1Id, zodiac2Id) {
        try {
            // compatibilityData가 로드되지 않았으면 로드
            if (!this.compatibilityData) {
                console.log('⏳ compatibilityData가 없음, loadCompatibilityData() 호출...');
                await this.loadCompatibilityData();
            }

            // 데이터가 여전히 없으면 null 반환
            if (!this.compatibilityData) {
                console.log('❌ compatibilityData 로드 실패');
                return null;
            }

            // 항상 작은 ID가 먼저 오도록 정규화
            const minId = Math.min(zodiac1Id, zodiac2Id);
            const maxId = Math.max(zodiac1Id, zodiac2Id);
            const key = `${minId}-${maxId}`;

            console.log(`🗄️ compatibilityData에서 조회: ${key}`);

            const data = this.compatibilityData[key];
            if (data) {
                console.log(`✅ compatibilityData에서 데이터 발견: ${key}`);
                // compat_message를 description으로 사용
                return {
                    ...data,
                    description: data.compat_message || data.description
                };
            }

            console.log('❌ compatibilityData에 데이터 없음:', key);
            return null;
        } catch (error) {
            console.error('❌ compatibilityData 조회 실패:', error);
            return null;
        }
    }

    /**
     * zodiacDescriptions에서 특정 별자리의 궁합 텍스트만 추출하고 길이 제한
     */
    extractCompatibilityText(zodiacDescriptions, zodiac1Id, zodiac2Id) {
        if (!zodiacDescriptions) return null;

        const zodiac1Data = zodiacDescriptions[zodiac1Id];
        const zodiac2Data = zodiacDescriptions[zodiac2Id];
        const zodiac1Name = this.zodiacSigns[zodiac1Id - 1]?.name;
        const zodiac2Name = this.zodiacSigns[zodiac2Id - 1]?.name;

        let extractedText = null;

        // 첫 번째 별자리의 궁합 텍스트에서 상대 별자리 관련 부분 찾기
        if (zodiac1Data?.compatibility && zodiac2Name) {
            const text = zodiac1Data.compatibility;

            // 상대방 별자리명이 포함된 부분 찾기
            const nameIndex = text.indexOf(zodiac2Name);
            if (nameIndex !== -1) {
                // 해당 별자리 섹션 추출
                const startIndex = Math.max(0, text.lastIndexOf('<div class="compat-item">', nameIndex));
                const endIndex = text.indexOf('</div>', nameIndex);

                if (startIndex !== -1 && endIndex !== -1) {
                    let section = text.substring(startIndex, endIndex + 6);
                    ...
                    console.log(`🎨 ${zodiac1Name} → ${zodiac2Name} 궁합 텍스트 추출 완료 (${section.length}자)`);
                }
            }
        }

        // 두 번째 별자리에서도 시도 (첫 번째에서 못 찾은 경우)
        if (!extractedText && zodiac2Data?.compatibility && zodiac1Name) {
            const text = zodiac2Data.compatibility;
            const nameIndex = text.indexOf(zodiac1Name);
            ...
            if (nameIndex !== -1) {
                ...
                if (startIndex !== -1 && endIndex !== -1) {
                    ...
                    extractedText = section;
                    console.log(`🎨 ${zodiac2Name} → ${zodiac1Name} 궁합 텍스트 추출 완료 (${section.length}자)`);
                }
            }
        }
    ...
    async generateCompatibility(zodiac1Id, zodiac2Id) {
        console.log('🔄 generateCompatibility() 호출:', { zodiac1Id, zodiac2Id });
        ...
        // compatibilityData에서 데이터 찾기
        if (this.compatibilityData) {
            const key = `${zodiac1Id}-${zodiac2Id}`;
            const data = this.compatibilityData[key];

            if (data) {
                console.log('✅ compatibility-data.json에서 데이터 발견:', key);
                return {
                    zodiac1Id: zodiac1Id,
                    zodiac2Id: zodiac2Id,
                    score: data.overall_score,
                    description: data.compat_message || data.description,  // 긴 메시지 우선 사용
                    details: {
                        love: `애정 궁합: ${data.love_score}점`,
                        friendship: `우정 궁합: ${data.friendship_score}점`,
                        work: `업무 궁합: ${data.work_score}점`
                    },
                    advice: data.advice
                };
            } else {
                console.warn('⚠️ 조합을 찾을 수 없음:', key);
            }
        }

        // 폴백: 기본 데이터 반환 (데이터 없을 경우)
        console.log('⚠️ 폴백 모드: 기본 궁합 데이터 생성');
        return {
            zodiac1Id: zodiac1Id,
            zodiac2Id: zodiac2Id,
            score: 75,
